...
import { pipe, prop, map, evolve, path } from 'ramda'
...
const convertRepositoryTopics = pipe(prop('nodes'), map(convertRepositoryTopic))
...
const fetchScreenshots = async (repo) => {
  ...
  try {
    const screenshotRes = await axios(
      `https://api.github.com/repos/milogert/${repo.name}/contents/screenshots`,
    )
...
export const getPinnedRepos = async () => {
  try {
    ...
    return {
      error: {
        message:
          'Oops! For some reason the pinned repositories were unavailable...',
        code: `${error.code}`,
      },
    }
  }
